...
unsigned long targetClickTime = 250; // number of ms between encoder clicks, so higher = slower
...
void updateTiming()
{
  ...
  if(avgClickTime < 0) avgClickTime = 0.5 * targetClickTime;
  if(avgClickTime > 3 * targetClickTime) avgClickTime = 3 * targetClickTime;
}
